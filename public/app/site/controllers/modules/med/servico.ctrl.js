...
.controller('MedServCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, EspSrv, ServSrv, InfoSrv ) {
    
    // VAR 
    var self = this
    $scope.confModal = {
        templates: 'tpls/admin/modules/med/modal/servico.mod.html',
        templaDel: 'tpls/admin/modules/med/modal/delete.serv.mod.html',
        titles: 'Serviços',
        size: 'lg'
    };
    // Text Conf
    ...
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        ServSrv.get({segment: SEGMENTS.MED},function(response){
    ...
    $scope.actionUpdate = function( data ) {
        return Modal.openModal({
                ...
                callback: function($scope) {
                    ...
                    $scope.serv = {};
                    $scope.serv = ServSrv.show({id: data.id });
                }
            }
        );
    };
    ...
    $scope.save = function() {
        if ($scope.servForm.$valid) {
            var promise =  $scope.serv.$save();
    ...
    $scope.destroy = function(id) {
        var promise =  ServSrv.destroy(id);
        promise.then(function(){
            self.reload();
            $timeout(function(){
                $scope.cancel();
