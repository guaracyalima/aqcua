...
.controller('ShopGaleriasCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, GaleriasSrv, InfoSrv ) {
    
    // VAR 
    var self = this
    $scope.confModal = {
        templates: 'tpls/admin/modules/shop/modal/galerias.mod.html',
        templaDel: 'tpls/admin/modules/shop/modal/galerias.delete.mod.html',
        titles: 'Galerias',
        size: 'lg'
    };
    // Text Conf
    ...
    // GET PAGE
    $scope.page = null;
    GaleriasSrv.get({type: 'request', cmd: 'pagina', segment: SEGMENTS.SHOP},function(response){
        $scope.page = response;
    });
    // DATATABLES
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        GaleriasSrv.get({segment: SEGMENTS.SHOP},function(response){
    ...
    function actionNew(){
        return Modal.openModal({
                ...
                callback: function($scope) {
                    $scope.galerias = new GaleriasSrv;
                }
            }
        );
    }
    $scope.actionUpdate = function( data ) {
        return Modal.openModal({
                ...
                callback: function($scope) {
                    $scope.galerias = {};
                    $scope.galerias = GaleriasSrv.show({id: data.id });
                }
            }
        );
    };
    ...
    $scope.save = function() {
        if ($scope.galeriasForm.$valid) {
            if( $scope.obj.src ){
                $scope.galerias.img = $scope.obj
            }
            $scope.galerias['page_id'] =  $scope.page.id;
            var promise =  $scope.galerias.$save();
    ...
    $scope.destroy = function(id) {
        var promise =  GaleriasSrv.destroy(id);
        promise.then(function(){
            self.realod();
            $timeout(function(){
                $scope.cancel();
