...
.controller('GrupoServicosCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, ContentPagesSrv, BoxContentsSrv, ServicosSrv, InfoSrv ) { //GrupoHomeSrv
    
    // VAR 
    var self = this;
    $scope.confModal = {
        templates: 'tpls/admin/modules/grupo/modal/servicos.mod.html',
        titles: 'Serviços',
        size: 'lg'
    };
    // Text Conf
    ...
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        ServicosSrv.get({segment: SEGMENTS.GRUPO},function(response){
    ...
    $scope.actionUpdate = function( data ) {
        return Modal.openModal({
                ...
                callback: function($scope) {
                    ...
                    // GET INFOR
                    $scope.servicos = {};
                    ServicosSrv.show({id: data.id }, function(response){
                        $scope.servicos = response;
                    });
                    ContentPagesSrv.get({type: 'ref', cmd: data.id, id_seg: SEGMENTS.GRUPO }, function( response ){
                    ...
                    BoxContentsSrv.getArray({type: 'ref', cmd: data.id, id_seg: SEGMENTS.GRUPO }, function( data ){
    ...
    $scope.save = function() {
        if ($scope.servicosForm.$valid) {
            $scope.servicos.content.title = 'Serviços';
