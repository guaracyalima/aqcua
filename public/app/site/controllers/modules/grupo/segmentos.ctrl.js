...
.controller('GrupoSegmentosCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, BannersSrv, PagesSrv, InfoSrv ) {
    
    // VAR 
    var self = this;
    $scope.confModal = {
        templates: 'tpls/admin/modules/grupo/modal/segmentos.mod.html',
        templaDel: 'tpls/admin/modules/grupo/modal/segmentos.delete.mod.html',
        titles: 'Banners da pagina inicial',
        size: 'lg'
    };
    // Text Conf
    $scope.taConf = [
    ...
    // GET PAGE
    $scope.page = null;
    BannersSrv.get({type: 'request', cmd: 'pagina', segment: SEGMENTS.GRUPO},function(response){
        $scope.page = response;
    });
    // DATATABLES
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        BannersSrv.get({segment: SEGMENTS.GRUPO},function(response){
    ...
    $scope.actionUpdate = function( data ) {
        return Modal.openModal({
                ...
                callback: function($scope) {
                    $scope.banners = {};
                    $scope.banners = BannersSrv.show({id: data.id });
                }
            }
        );
    };
    ...
    $scope.save = function() {
        if ($scope.bannersForm.$valid) {
            if( $scope.obj.src ){
                $scope.banners.img = $scope.obj
            }
            $scope.banners['page_id'] =  $scope.page.id;
            var promise =  $scope.banners.$save();
    ...
    $scope.destroy = function(id) {
        var promise =  BannersSrv.destroy(id);
        promise.then(function(){
            self.realod();
            $timeout(function(){
                $scope.cancel();
