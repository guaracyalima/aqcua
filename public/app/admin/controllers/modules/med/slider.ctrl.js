...
.controller('MedSliderCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, SliderSrv, InfoSrv ) {
    
    // VAR 
    var ctrl = this
    $scope.confModal = {
        templates: 'tpls/admin/modules/med/modal/slider.mod.html',
        templaDel: 'tpls/admin/modules/med/modal/slider.delete.mod.html',
        titles: 'Slider',
        size: 'lg'
    };
    // Text Conf
    ...
    // GET PAGE
    $scope.page = null;
    SliderSrv.get({type: 'request', cmd: 'pagina', segment: SEGMENTS.MED},function(response){
        $scope.page = response;
    });
    // DATATABLES
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        SliderSrv.get({segment: SEGMENTS.MED},function(response){
