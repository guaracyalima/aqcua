...
.controller('MedEspCtrl', function( $scope ,$state, $rootScope, $timeout, $compile, $q, ngToast, Modal, DTOptionsBuilder, DTColumnBuilder, DTABLE, SEGMENTS, EspSrv, InfoSrv ) {
    
    // VAR 
    var self = this
    $scope.confModal = {
        templates: 'tpls/admin/modules/med/modal/especialidades.mod.html',
        templaDel: 'tpls/admin/modules/med/modal/delete.mod.html',
        titles: 'Esepecialidades',
        size: 'md'
    };
    // Text Conf
    $scope.taConf = [
    ...
    $scope.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        EspSrv.get({segment: SEGMENTS.MED},function(response){
    ...
    $scope.dtColumns = [
        ...
        DTColumnBuilder.newColumn('name').withTitle('Especialidade'),
    ...
    $scope.actionUpdate = function( data ) {
        return Modal.openModal({
                ...
                callback: function($scope) {
                    $scope.espec = {};
                    $scope.espec = EspSrv.show({id: data.id });
                }
            }
        );
    };
    ...
    $scope.save = function() {
        if ($scope.especForm.$valid) {
            $scope.espec.seg_id =  SEGMENTS.MED;
            var promise =  $scope.espec.$save();
